include $(SOURCE_DIR)/mirror/redhat/yum_repos.mk

.PHONY: show-yum-urls-redhat

$(BUILD_DIR)/mirror/redhat/etc/yum.conf: $(call depv,yum_conf)
$(BUILD_DIR)/mirror/redhat/etc/yum.conf: export contents:=$(yum_conf)
$(BUILD_DIR)/mirror/redhat/etc/yum.conf:
	mkdir -p $(@D)
	/bin/echo -e "$${contents}" > $@

$(BUILD_DIR)/mirror/redhat/etc/yum-plugins/priorities.py: \
		$(SOURCE_DIR)/mirror/redhat/yum-priorities-plugin.py
	mkdir -p $(@D)
	cp $(SOURCE_DIR)/mirror/redhat/yum-priorities-plugin.py $@

$(BUILD_DIR)/mirror/redhat/etc/yum/pluginconf.d/priorities.conf:
	mkdir -p $(@D)
	/bin/echo -e "[main]\nenabled=1\ncheck_obsoletes=1\nfull_match=1" > $@

$(BUILD_DIR)/mirror/redhat/etc/yum.repos.d/base.repo: $(call depv,YUM_REPOS)
$(BUILD_DIR)/mirror/redhat/etc/yum.repos.d/base.repo: \
		export contents:=$(foreach repo,$(YUM_REPOS),\n$(yum_repo_$(repo))\n)
$(BUILD_DIR)/mirror/redhat/etc/yum.repos.d/base.repo:
	@mkdir -p $(@D)
	/bin/echo -e "$${contents}" > $@

$(BUILD_DIR)/mirror/redhat/etc/yum.repos.d/extra.repo: $(call depv,EXTRA_RPM_REPOS)
$(BUILD_DIR)/mirror/redhat/etc/yum.repos.d/extra.repo: \
...
$(BUILD_DIR)/mirror/redhat/yum.done: \
	...
	rm -rf $(LOCAL_MIRROR_REDHAT_OS_BASEURL)/Packages/*.i686.rpm
	# Yumdownloader workaround number two:
	# yumdownloader should fail if some packages are missed
	test `grep "No Match" $(BUILD_DIR)/mirror/redhat/yumdownloader.log | wc -l` = 0
	...
	test `grep "conflicts with" $(BUILD_DIR)/mirror/redhat/yumdownloader.log | grep -v '^[[:space:]]' | wc -l` -le 9
	# Yumdownloader workaround number four:
	# yumdownloader should fail if some errors appears
	test `grep "Errno" $(BUILD_DIR)/mirror/redhat/yumdownloader.log | wc -l` = 0
	$(ACTION.TOUCH)
...
$(LOCAL_MIRROR_REDHAT_OS_BASEURL)/comps.xml:
	...
	if ( echo $${COMPSXML} | grep -q '\.gz$$' ); then \
		wget -O $@.gz $(MIRROR_REDHAT_OS_BASEURL)/$${COMPSXML}; \
		gunzip $@.gz; \
	else \
		wget -O $@ $(MIRROR_REDHAT_OS_BASEURL)/$${COMPSXML}; \
	fi

$(BUILD_DIR)/mirror/redhat/repo.done: \
		$(BUILD_DIR)/mirror/redhat/yum.done \
		| $(LOCAL_MIRROR_REDHAT_OS_BASEURL)/comps.xml
	createrepo -g $(LOCAL_MIRROR_REDHAT_OS_BASEURL)/comps.xml \
		-o $(LOCAL_MIRROR_REDHAT_OS_BASEURL)/ $(LOCAL_MIRROR_REDHAT_OS_BASEURL)/
	$(ACTION.TOUCH)
